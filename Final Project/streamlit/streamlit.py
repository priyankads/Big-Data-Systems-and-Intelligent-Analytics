...
with st.sidebar:
    ...
    link = "CLAAT [link](https://codelabs-preview.appspot.com/?file_id=12z0C4Hw5840SwWBf6vqpAS81ir7TEm-NcLhjHyZOQW4#12)"
    st.markdown(link,unsafe_allow_html=True)
    #st.write("CLAAT [link](https://codelabs-preview.appspot.com/?file_id=12z0C4Hw5840SwWBf6vqpAS81ir7TEm-NcLhjHyZOQW4#12)")
...
if choose == "Home":
    ...
    if st.session_state.useradmin == 'admin':
        ...
        # overall_average_rating = round(((avg_rating_loved_it+avg_rating_like_it+avg_rating_okay+avg_rating_hate_it)/4), 1)
        total_sum = user_data["loved_it"].sum()+user_data["like_it"].sum()+user_data["okay"].sum()+user_data["hate_it"].sum()
        overall_average_rating = (user_data["loved_it"].sum()/total_sum)*10
        star_rating = ":star:" * int(round(overall_average_rating, 0))
        # get latest timestamp from dataframe 
        last_accessed_timestamp = user_data["Timestamp"].max()
